 ...
 * 具体来说，主要问题就是遇到右括号时如何判断当前的合法序列的长度。比较健壮的方式如下： (1)
...
public class LongestValidParentheses {
	public int longestValidParentheses(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			if (s.charAt(i) == '(') {
				stack.push(i);// 如果是‘（ ’， 把位置放到stack中去
			} else {
				if (stack.isEmpty()) {
					accumulatedLength = 0;// 如果当前栈为空，则说明加上当前右括号没有合法序列
				} else {
					int matchedPosition = stack.pop();
					int matchedLength = i - matchedPosition + 1;

					//如果pop出来之后stack是空的，加到accumulated的长度里去
					if (stack.isEmpty()) {
